...
import { gradeAttemptAction } from "@/app/actions/trainer";
...
export default async function GradeAttempt({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string }> }) {
  ...
  return (
    <Container className="max-w-4xl">
      <PageHeader
        back={{ href: "/trainer/grading", label: "Corrections" }}
        title={`Quiz : ${attempt.quiz.lesson.title}`}
        subtitle={
          <>
            <Link href={`/trainer/courses/${courseId}/learners/${attempt.user.id}`} className="hover:underline">{attempt.user.name}</Link> · {attempt.quiz.lesson.module.course.title} · remis le {formatDate(attempt.submittedAt, true)}
          </>
        }
      />
