...
import { forceCompleteLessonAction, resetQuizAttemptsAction } from "@/app/actions/trainer";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <Link href={`/trainer/courses/${id}/learners`} className="text-sm text-slate-500 hover:text-brand-600">← Apprenants</Link>
                  ...
                  return (
                    <tr key={l.id}>
                      ...
                      <td>
                        {r ? (
                          r.refId ? (
                            <Link className="text-brand-600 hover:underline" href={r.kind === "QUIZ" ? `/trainer/grading/attempts/${r.refId}` : `/trainer/grading/submissions/${r.refId}`}>
                              {pct(r.percent)}
                            </Link>
                          ) : "—"
