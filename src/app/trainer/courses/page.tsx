  ...
  return (
    <Container>
      <PageHeader
        ...
        actions={<Link href="/trainer/courses/new" className="btn-primary">+ Nouvelle formation</Link>}
      />
      {courses.length === 0 ? (
        <Empty title="Aucune formation">
          <Link href="/trainer/courses/new" className="text-brand-600 hover:underline">Créer ma première formation</Link>
        </Empty>
      ) : (
            ...
            <tbody>
              {courses.map((c) => (
                <tr key={c.id} className="hover:bg-slate-50">
                  <td>
                    <Link href={`/trainer/courses/${c.id}`} className="font-medium text-slate-900 hover:text-brand-700">{c.title}</Link>
