...
export default async function Grading({ searchParams }: { searchParams: Promise<{ course?: string; tab?: string }> }) {
  ...
  const items = [
    ...submissions.map((s) => ({
      id: s.id, kind: "Devoir" as const, href: `/trainer/grading/submissions/${s.id}`, learner: s.user.name, title: s.lesson.title,
      course: s.lesson.module.course.title, date: s.submittedAt, percent: s.percent, status: s.status,
    })),
    ...attempts.map((a) => ({
      id: a.id, kind: "Quiz" as const, href: `/trainer/grading/attempts/${a.id}`, learner: a.user.name, title: a.quiz.lesson.title,
  ...
  const q = (t: string) => `/trainer/grading?tab=${t}${course ? `&course=${course}` : ""}`;
  return (
    <Container>
