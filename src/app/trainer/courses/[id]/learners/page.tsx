...
import { enrollLearnersAction, removeEnrollmentAction, setEnrollmentStatusAction } from "@/app/actions/trainer";
            ...
            <table className="table">
              ...
              <tbody>
                {rows.map(({ e, percent, average, pending }) => (
                  <tr key={e.id}>
                    <td>
                      <Link href={`/trainer/courses/${id}/learners/${e.userId}`} className="font-medium hover:text-brand-700">{e.user.name}</Link>
                    ...
                    <td className="whitespace-nowrap text-right">
                      <Link href={`/trainer/courses/${id}/learners/${e.userId}`} className="btn-ghost btn-sm">Détail</Link>
                      {e.status === "SUSPENDED" ? (
