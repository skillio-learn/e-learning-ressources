  ...
  return (
    <Container>
      <PageHeader
        ...
        actions={<Link href="/of/rubrics/new" className="btn-primary">Créer une grille</Link>}
      />
      {rubrics.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {rubrics.map((r) => (
            <Link key={r.id} href={`/of/rubrics/${r.id}`} className="card flex flex-col gap-2 p-5">
              ...
              <div className="text-xs text-slate-500">par {r.author.name} · {formatDate(r.updatedAt)}</div>
            </Link>
          ))}
        </div>
      )}
