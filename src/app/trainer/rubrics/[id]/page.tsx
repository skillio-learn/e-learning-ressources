...
import { deleteRubricAction, duplicateRubricAction } from "@/app/actions/trainer";
...
export default async function RubricPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <Container className="max-w-6xl">
      <PageHeader
        back={{ href: "/trainer/rubrics", label: "Grilles" }}
        title={rubric.title}
              ...
              {lessons.map((l) => (
                <li key={l.id}>
                  {manageableIds.includes(l.module.courseId) ? (
                    <Link href={`/trainer/courses/${l.module.courseId}/lessons/${l.id}`} className="text-brand-600 hover:underline">{l.title}</Link>
                  ) : l.title}{" "}
                ...
                <tbody>
                  {submissions.map((s) => (
                    <tr key={s.id}>
                      ...
                      <td className="text-right"><Link href={`/trainer/grading/submissions/${s.id}`} className="btn-ghost btn-sm">Ouvrir</Link></td>
                    </tr>
                  ))}
                </tbody>
              </table>
