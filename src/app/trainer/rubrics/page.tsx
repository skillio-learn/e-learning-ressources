  ...
  return (
    <Container>
      <PageHeader
        ...
        actions={<Link href="/trainer/rubrics/new" className="btn-primary">+ Nouvelle grille</Link>}
      />
      {rubrics.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {rubrics.map((r) => (
            <Link key={r.id} href={`/trainer/rubrics/${r.id}`} className="card flex flex-col gap-2 p-5 hover:shadow-md">
