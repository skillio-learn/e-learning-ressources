...
export function CourseCard({
  ...
  return (
    <Link href={href} className="card card-hover group flex flex-col overflow-hidden">
      <div className="relative aspect-[16/9] overflow-hidden bg-[#0b0b10]">
        {course.coverUrl ? (
          ...
          <>
            <div className="absolute -left-10 -top-16 h-48 w-48 rounded-full bg-[#2997ff]/40 blur-3xl transition duration-700 group-hover:translate-x-6" />
            <div className="absolute -bottom-20 right-0 h-56 w-56 rounded-full bg-[#bf5af2]/35 blur-3xl transition duration-700 group-hover:-translate-x-6" />
            <div className="bg-grid absolute inset-0 opacity-60" />
            <div className="relative grid h-full place-items-center p-6 text-center font-display text-lg font-semibold leading-snug tracking-tight text-white">
              {course.title}
            </div>
          </>
        )}
