...
export default async function LearnLayout({ children, params }: { children: React.ReactNode; params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="mx-auto flex max-w-[1600px] flex-col lg:flex-row">
      <aside className="no-print border-b border-slate-200 bg-white lg:sticky lg:top-14 lg:h-[calc(100vh-3.5rem)] lg:w-80 lg:shrink-0 lg:overflow-y-auto lg:border-b-0 lg:border-r">
        <div className="border-b border-slate-100 p-4">
          <Link href={`/learn/${course.slug}`} className="font-semibold text-slate-900 hover:text-brand-700">
            {course.title}
          </Link>
