      ...
      <section className="border-b border-slate-200 bg-surface">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-16 md:grid-cols-[1fr_360px]">
          <div className="animate-fade-up">
            ...
            <h1>{course.title}</h1>
            {course.subtitle && <p className="mt-4 max-w-[65ch] text-lg leading-8 text-slate-500">{course.subtitle}</p>}
            ...
            <div className="mt-8 flex flex-wrap gap-x-6 gap-y-2 text-sm text-slate-700">
              {(
                ...
                <span key={label} className="inline-flex items-center gap-2">
                  <Icon className="h-5 w-5 text-brand-600" strokeWidth={1.75} aria-hidden="true" /> {label}
                </span>
              ))}
            </div>
          </div>
          <div className="card self-start p-6 text-slate-800">
            {course.coverUrl && (
                  ...
                  <ol className="border-t border-slate-100 px-4 py-2">
                    {m.lessons.map((l, j) => (
                      <li key={l.id} className="flex items-center gap-3 py-1.5 text-sm">
                        <span className="w-6 text-right text-slate-500">{j + 1}.</span>
                        <LessonTypeIcon type={l.type} />
                        <span className="flex-1">{l.title}</span>
                        {l.durationMin ? <span className="text-xs text-slate-500">{l.durationMin} min</span> : null}
                      </li>
                    ))}
                  </ol>
                </details>
