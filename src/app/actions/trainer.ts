...
import type { CompletionMode, CourseLevel, CourseStatus, EnrollmentPolicy, LessonType, QuestionType } from "@prisma/client";
...
const staff = () => requireRole("ADMIN", "TRAINER");
...
export async function createCourseAction(fd: FormData) {
  ...
  const course = await db.course.create({
    data: { ...data, slug: await uniqueSlug(data.title), authorId: user.id },
  });
...
export async function updateCourseAction(courseId: string, fd: FormData) {
  ...
  await db.course.update({ where: { id: courseId }, data: { ...data, slug } });
  revalidatePath(`/trainer/courses/${courseId}`, "layout");
}
...
export async function duplicateCourseAction(courseId: string) {
  ...
  redirect(`/trainer/courses/${copy.id}`);
}
...
export async function addCoTrainerAction(courseId: string, fd: FormData) {
  ...
  revalidatePath(`/trainer/courses/${courseId}/settings`);
}
...
export async function addLessonAction(moduleId: string, fd: FormData) {
  ...
  redirect(`/trainer/courses/${courseId}/lessons/${lesson.id}`);
}
...
export async function updateQuizSettingsAction(quizId: string, fd: FormData) {
  ...
  revalidatePath(`/trainer/courses/${courseId}/lessons/${quiz.lessonId}`);
}
...
export async function deleteQuestionAction(questionId: string) {
  ...
  revalidatePath(`/trainer/courses/${courseId}/lessons/${q.quiz.lessonId}`);
}
...
export async function enrollLearnersAction(courseId: string, _prev: EnrollResult, fd: FormData): Promise<EnrollResult> {
  ...
  for (const line of lines) {
    ...
    if (!learner) {
      ...
      learner = await db.user.create({ data: { email, name, passwordHash: await bcrypt.hash(password, 10), role: "LEARNER" } });
  ...
  revalidatePath(`/trainer/courses/${courseId}/learners`);
...
export async function setEnrollmentStatusAction(enrollmentId: string, status: "ACTIVE" | "SUSPENDED") {
  ...
  revalidatePath(`/trainer/courses/${e.courseId}/learners`);
}
...
export async function forceCompleteLessonAction(courseId: string, userId: string, lessonId: string) {
  ...
  revalidatePath(`/trainer/courses/${courseId}/learners/${userId}`);
}
...
export async function gradeAttemptAction(attemptId: string, fd: FormData) {
  ...
  redirect(`/trainer/grading/attempts/${attemptId}?saved=1`);
}
...
export async function gradeSubmissionAction(submissionId: string, fd: FormData) {
  ...
  revalidatePath(`/trainer/grading`);
  redirect(`/trainer/grading/submissions/${submissionId}?saved=1`);
}
...
export async function saveRubricAction(rubricId: string | null, input: RubricInput, attachToLessonId?: string | null) {
  ...
  if (rubricId) {
    ...
    revalidatePath("/trainer/rubrics");
    return { id: rubricId };
  }
...
export async function duplicateRubricAction(rubricId: string) {
  ...
  redirect(`/trainer/rubrics/${copy.id}`);
}
