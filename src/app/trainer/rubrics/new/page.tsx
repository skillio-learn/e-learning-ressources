...
export default async function NewRubric({ searchParams }: { searchParams: Promise<{ courseId?: string; lessonId?: string }> }) {
  ...
  return (
    <Container className="max-w-5xl">
      <PageHeader title="Nouvelle grille d'évaluation" back={{ href: "/trainer/rubrics", label: "Grilles" }} />
      <RubricEditor
        rubricId={null}
        courses={courses}
        attachToLessonId={validCourse ? lessonId : null}
        returnTo={validCourse && lessonId ? `/trainer/courses/${validCourse}/lessons/${lessonId}` : null}
        initial={{
          title: "",
          description: "",
